...
const data = req.query.date;
    axios.get(`https://cla-pixel.lgads.tv/pixels?deviceId=${deviceId}&date=2024-01-01`, { headers })
      .then(async data => {
      ...
      const promises = data.data.map((pixelItem, index) => {//promises array of all api calls
        ...
        return axios.get(`https://cla-epg.lgads.tv/epg/listings?stationId=${pixelItem.stationId}`, { headers }).then((data) => {
          return data.data;
        });
      })
      console.log("Pixel promisees...", promises);
      ...
      res.status(200).json(recommendedEpgList)
      }).catch(err => {
        console.log(err);
